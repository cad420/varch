...
#include "block_input_streamer.hpp"
...
private:
	RefinerOptions opts;
	const size_t block_size, block_inner, block_len;
	index::Idx dim;
	index::Idx adjusted;

	// RawReaderIO input;
	std::unique_ptr<BlockInputStreamer> input;
	ofstream output;
...
public:
	RefinerImpl( RefinerOptions const &_ ) :
	  opts( _ ),
	  block_size( 1 << opts.log_block_size ),
	  block_inner( block_size - 2 * opts.padding ),
	  block_len( block_size * block_size * block_size ),
	  dim(),
	  adjusted( index::Idx{}
	...
	{
		if ( opts.padding < 0 || opts.padding > 2 ) {
			std::cout << "Unsupported padding\n";
			return;
		}

		if ( !opts.sub_region_begin.has_value() ) {
			opts.sub_region_begin = index::Idx{};
		}
		if ( !opts.sub_region_size.has_value() ) {
			opts.sub_region_size =
			  index::Idx{}
				.set_x( opts.raw_size.x - opts.sub_region_begin.value().x )
				.set_y( opts.raw_size.y - opts.sub_region_begin.value().y )
				.set_z( opts.raw_size.z - opts.sub_region_begin.value().z );
		}
		input.reset(
		  new BlockInputStreamer(
			BlockInputStreamerOptions{}
			  .set_path( opts.input )
			  .set_raw_size( opts.raw_size )
			  .set_sub_region_begin( opts.sub_region_begin.value() )
			  .set_sub_region_size( opts.sub_region_size.value() )
			  .set_voxel_size( sizeof( Voxel ) )
			  .set_block_size( block_size )
			  .set_padding( opts.padding ) ) );

		// vm::println( "raw: {}", opts.raw_size );
	...
	bool convert( ConvertOptions const &opts )
	{
		...
		auto mempool = BlockMempool( block_len, 2 );
		auto input_stream = input->stream( mempool );

		rt.spawn(
		  std::move( input_stream )
			.for_each(
			  [&]( BlockMempool::Block block ) {
				  vm::println( "got one block" );
			  } ) );
		...
		// 	int nblocks_in_mem = mem_size_in_bytes / block_size_in_bytes;
		// 	// / 2 /*two buffers*/;
		...
		// BlockMempool block_pool( nvoxels_per_block, nblocks_per_stride );
